...
for k, v in {
    ...
    "query_count": 0,
    "pipeline_log": [],
    "active_step": 0,          # 0=upload 1=convert 2=transcribe 3=embed 4=search
}.items():
...
.stApp::before {
    ...
    background: radial-gradient(circle, #0d3a2e44 0%, transparent 70%);
...
.stApp::after {
    ...
    background: radial-gradient(circle, #1a1a4a33 0%, transparent 70%);
    pointer-events: none; z-index: 0;
}
...
.hero-title {
    ...
    font-weight: 800;
    line-height: 1.05;
    background: linear-gradient(120deg, #2acea8 0%, #5ba4f5 50%, #a78bfa 100%);
...
.step-circle {
    ...
    border: 1.5px solid #151f30;
    background: #090e18;
    color: #1e2d40;
}
.step-circle.done   { border-color: #2acea8; color: #2acea8; background: #071912; }
.step-circle.active { border-color: #5ba4f5; color: #5ba4f5; background: #080f1e;
                      box-shadow: 0 0 20px #5ba4f530; }
.step-label {
    font-size: 9px; letter-spacing: 2px;
    text-transform: uppercase; color: #1e2d40;
    margin-top: 7px; text-align: center; font-weight: 500;
}
...
.step-line {
    flex: 1; height: 1px;
    background: #151f30;
    margin-bottom: 20px; min-width: 24px;
}
...
.panel::after {
    ...
    background: linear-gradient(90deg, transparent, #2acea840, #5ba4f540, #a78bfa40, transparent);
}
...
.chip::before {
    ...
    background: linear-gradient(90deg, #2acea840, transparent);
}
.chip-val {
    font-family: 'Syne', sans-serif;
    font-size: 26px; color: #5ba4f5; font-weight: 800;
    line-height: 1.1;
}
.chip-lbl {
    font-size: 9px; letter-spacing: 2.5px;
    text-transform: uppercase; color: #253040;
...
.res-card:hover {
    border-color: #2acea850;
    box-shadow: 0 4px 24px #2acea810;
}
.res-header {
...
.res-title {
    font-family: 'Syne', sans-serif;
    font-size: 12px; color: #5ba4f5;
    letter-spacing: 0.5px; font-weight: 700;
    line-height: 1.4;
}
.res-score {
    font-family: 'Courier New', monospace;
    font-size: 12px; color: #2acea8;
    background: #071912; border: 1px solid #0f3028;
...
/* ── AI response — rendered via st.markdown, so target stMarkdown ── */
div[data-testid="stMarkdownContainer"] .ai-response-wrap {
    background: #060b16;
    border: 1px solid #111d30;
    border-radius: 16px;
    padding: 28px 32px;
...
div[data-testid="stMarkdownContainer"] .ai-response-wrap::before {
    content: '';
    position: absolute; top: 0; left: 0; bottom: 0; width: 3px;
    background: linear-gradient(180deg, #2acea8, #5ba4f5, #a78bfa);
    border-radius: 3px 0 0 3px;
}

/* Target markdown elements rendered INSIDE streamlit natively */
.stMarkdown p   { margin: 0 0 10px !important; font-size: 14.5px; line-height: 1.8; color: #aabacf; }
.stMarkdown ul,
.stMarkdown ol  { margin: 2px 0 10px !important; padding-left: 20px; }
...
.stMarkdown h3  {
    ...
    color: #2acea8 !important; margin: 20px 0 10px !important;
    border-bottom: 1px solid #0f2820; padding-bottom: 6px;
}
.stMarkdown h2  {
    font-family: 'Syne', sans-serif !important;
    font-size: 13px !important; font-weight: 700 !important;
    color: #5ba4f5 !important; margin: 16px 0 8px !important;
}
.stMarkdown code {
    background: #0a1628 !important;
    border: 1px solid #152240 !important;
    border-radius: 4px !important; padding: 1px 7px !important;
    font-size: 12.5px !important; color: #5ba4f5 !important;
}
.stMarkdown blockquote {
    border-left: 2px solid #a78bfa !important;
    margin: 8px 0 !important; padding: 4px 14px !important;
    color: #7a8fa8 !important; font-style: italic;
}
...
section[data-testid="stSidebar"] {
    ...
    border-right: 1px solid #0e1826 !important;
}
...
div[data-testid="stFileUploader"] {
    ...
    border: 1px dashed #111d30 !important;
...
.stTextArea textarea:focus {
    border-color: #2acea840 !important;
    box-shadow: 0 0 0 2px #2acea810 !important;
}
...
.stTabs [data-baseweb="tab-list"] {
    background: transparent !important;
    border-bottom: 1px solid #111d30 !important;
    gap: 4px;
}
.stTabs [data-baseweb="tab"] {
    font-family: 'Syne', sans-serif !important;
    font-size: 10px !important; font-weight: 700 !important;
    letter-spacing: 3px !important; color: #253040 !important;
    background: transparent !important; border: none !important;
    text-transform: uppercase !important; padding: 8px 18px !important;
}
.stTabs [aria-selected="true"] {
    color: #2acea8 !important;
    border-bottom: 2px solid #2acea8 !important;
}
...
def check_ollama_status(model):
    try:
        requests.get("http://localhost:11434", timeout=2)
        r = requests.get("http://localhost:11434/api/tags", timeout=2)
        models = [m["name"] for m in r.json().get("models", [])]
        return "ready" if any(model in m for m in models) else "not_downloaded"
    except:
        return "offline"
...
def generate_response(prompt, model, temp):
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 🤖  MODEL")
    model = st.selectbox("LLM", ["llama3.2:latest"])
    temperature = st.slider("Temperature", 0.0, 1.0, 0.7)
    status = check_ollama_status(model)
    if status == "ready":
        st.markdown("""
        <div style="
            background: linear-gradient(135deg, #071912 0%, #0a1f18 100%);
        ...
        ">
            <div style="
                position: absolute; top: 0; left: 0; right: 0; height: 1px;
                background: linear-gradient(90deg, transparent, #2acea8, transparent);
            "></div>
            <div style="display:flex; align-items:center; gap:10px;">
                <div style="
                    width: 10px; height: 10px; border-radius: 50%;
                    background: #2acea8;
                    box-shadow: 0 0 8px #2acea8, 0 0 16px #2acea860;
                    flex-shrink: 0;
                    animation: pulse 2s infinite;
                "></div>
                <div>
                    <div style="font-family:'Syne',sans-serif; font-size:11px; font-weight:700;
                                letter-spacing:2px; color:#2acea8; text-transform:uppercase;">
                        Ollama Connected
                    </div>
                    <div style="font-size:10px; color:#1a5a45; margin-top:2px; letter-spacing:1px;">
                        bge-m3 · llama3.2 ready
                    </div>
                </div>
            </div>
        </div>
        <style>
        @keyframes pulse {
            0%, 100% { box-shadow: 0 0 8px #2acea8, 0 0 16px #2acea860; }
            50%       { box-shadow: 0 0 12px #2acea8, 0 0 28px #2acea8aa; }
        }
        </style>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <div style="
            background: linear-gradient(135deg, #1a1200 0%, #201800 100%);
        ...
        ">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
                ...
                <div>
                    ...
                    <div style="font-size:10px; color:#6a4e00; margin-top:2px; letter-spacing:1px;">
        ...
        st.markdown("""
        <div style="
            background: linear-gradient(135deg, #1a0608 0%, #200a0c 100%);
        ...
        ">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
                ...
                <div>
                    <div style="font-family:'Syne',sans-serif; font-size:11px; font-weight:700;
                                letter-spacing:2px; color:#f43f5e; text-transform:uppercase;">
                        Ollama Offline
                    </div>
                    <div style="font-size:10px; color:#6a1020; margin-top:2px; letter-spacing:1px;">
                        run: ollama serve
                    </div>
                </div>
            </div>
    ...
    if uploaded_files:
        ...
        for i, v in enumerate(sorted(all_vids)):
            ...
            <div style="background:#111827;border:1px solid #1e2d45;border-radius:8px;
                        padding:10px 14px;font-size:12px;color:#6eb4ff;
...
with st.container():
    ...
    videos_exist = os.path.exists("videos") and any(
        f.lower().endswith((".mp4", ".mkv", ".webm")) for f in os.listdir("videos")
    )
    audios_exist = os.path.exists("audios") and any(
        f.endswith(".mp3") for f in os.listdir("audios")
    )
    ...
    if convert_btn:
        ...
        for i, file in enumerate(video_files, start=1):
            ...
            try:
                subprocess.run(
                    ["ffmpeg", "-y", "-i", input_path, "-vn", "-ab", "192k", "-ar", "44100", mp3_path],
...
with st.container():
    ...
    jsons_exist = os.path.exists("jsons") and any(
        f.endswith(".json") for f in os.listdir("jsons")
    ) if os.path.exists("jsons") else False
    ...
    with c2:
        transcribe_btn = st.button("Transcribe", disabled=not (audios_exist or (os.path.exists("audios") and any(f.endswith(".mp3") for f in os.listdir("audios")))), use_container_width=True)
    ...
    if transcribe_btn:
        ...
        if not audio_list:
            st.warning("No properly named MP3s found. Run Step 2 first.")
        else:
            with st.spinner(f"Loading Whisper '{whisper_model_choice}' model…"):
                wmodel = _whisper.load_model(whisper_model_choice)
...
# ─────────────────────────────────────────────
with st.container():
    st.markdown('<div class="panel"><div class="panel-title">◈  Step 04 — Generate Embeddings (bge-m3)</div>', unsafe_allow_html=True)
    ...
    with c1:
        if emb_exists:
            ...
            st.markdown("Creates vector embeddings for every transcript chunk via Ollama bge-m3.")
    with c2:
        embed_btn = st.button(
            "Build Embeddings",
            disabled=not (os.path.exists("jsons") and any(f.endswith(".json") for f in os.listdir("jsons")) if os.path.exists("jsons") else False),
            use_container_width=True
        )

    if embed_btn:
        if check_ollama_status("bge-m3") == "offline":
...
# ─────────────────────────────────────────────
#  STEP 5 — SEARCH (original functionality)
# ─────────────────────────────────────────────
df = load_embeddings()
...
if df is None:
    ...
    <div style="text-align:center;padding:48px;color:#2e3a50;">
        <div style="font-family:'Space Mono',monospace;font-size:32px;margin-bottom:12px;">⬆</div>
        <div style="font-family:'Space Mono',monospace;font-size:13px;letter-spacing:2px;
                    color:#3de8c0;">Complete the pipeline above to unlock search</div>
    </div>
    """, unsafe_allow_html=True)
else:
    # DB overview chips
    ...
    <div class="chips-row">
        <div class="chip">
            <div class="chip-val">{len(df)}</div>
            <div class="chip-lbl">Total Chunks</div>
        </div>
        <div class="chip">
            <div class="chip-val">{df["title"].nunique()}</div>
            <div class="chip-lbl">Videos Indexed</div>
        </div>
        <div class="chip">
            <div class="chip-val">{int(df["text"].str.len().mean())}</div>
            <div class="chip-lbl">Avg Chunk Len</div>
        </div>
        <div class="chip">
            <div class="chip-val">{int(len(df) * 5)}</div>
            <div class="chip-lbl">Est. Seconds</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if final_query:
        ...
        if results.empty:
            ...
            st.markdown(f'<div class="panel-title" style="margin-top:28px;">◈  RELEVANT CHUNKS  ({len(results)} found)</div>',
                        unsafe_allow_html=True)
...
### 📖 Explanation
Write 3–5 paragraphs explaining the concept deeply in simple, plain English. Break complex ideas down. No bullet walls — use flowing prose with occasional bullets only for lists of distinct items.

### 💡 Real-World Example
Give at least ONE concrete, relatable real-world analogy or example that makes the concept click for a beginner. Make it vivid and specific.

### 🔑 Key Takeaways
3–5 crisp bullet points summarising what the student must remember.

### 🎬 Where to Learn More
List each relevant video chunk with its title and timestamp in format:
- **[Video Title]** → ⏱ MM:SS – MM:SS

FORMATTING RULES:
- Use **bold** for important terms when first introduced.
- Keep paragraphs tight (3–4 sentences max). No blank lines between list items.
- Do NOT pad with filler. Every sentence must add value.
- If the context doesn't cover the question, say exactly: "I couldn't find a detailed explanation in the uploaded videos."
...
"""
            ...
            st.markdown('<div class="panel-title" style="margin-top:28px;">◈  AI TEACHING RESPONSE</div>',
                        unsafe_allow_html=True)
            st.markdown("""
            <style>
            .ai-resp-wrap {
    ...
    if st.session_state.last_results is not None:
        ...
        col1.download_button("AI Response (.txt)",
                             st.session_state.ai_response, file_name="ai_response.txt")
        col2.download_button("Results (.csv)",
...
<div style='text-align:center;padding:60px 0 30px;'>
    <div style='font-family:"Syne",sans-serif;font-size:9px;
                letter-spacing:5px;color:#0e1826;text-transform:uppercase;font-weight:700;'>
        VIDMIND AI  ·  OLLAMA + WHISPER + BGE-M3  ·  © 2026
    </div>
</div>
""", unsafe_allow_html=True)